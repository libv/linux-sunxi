...
static struct clk *i2s0_apbclk 		= NULL;
static struct clk *i2s0_pll2clk 		= NULL;
...
void sunxi_snd_txctrl_i2s0(struct snd_pcm_substream *substream, int on)
{
	u32 reg_val;
...
void sunxi_snd_rxctrl_i2s0(struct snd_pcm_substream *substream, int on)
{
	u32 reg_val;	
...
static int sunxi_i2s0_set_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	...
	if (i2s0_select) {
		/*config as i2s0, the default register is i2s0.*/
...
{
	if (clk_set_rate(i2s0_pll2clk, freq)) {
		printk("try to set the i2s0_pll2clk failed!\n");
	}
...
static int __init sunxi_i2s0_dev_probe(struct platform_device *pdev)
{
	...
	if (clk_prepare_enable(i2s0_pllx8)) {
		printk("enable i2s0_pll2clk failed; \n");
	}

	/*i2s0 pll2clk*/
	i2s0_pll2clk = clk_get(NULL, "pll2");
	if ((!i2s0_pll2clk)||(IS_ERR(i2s0_pll2clk))) {
		printk("try to get i2s0_pll2clk failed\n");
	}
	if (clk_prepare_enable(i2s0_pll2clk)) {
		printk("enable i2s0_pll2clk failed; \n");
	}
	...
	if (clk_set_parent(i2s0_moduleclk, i2s0_pll2clk)) {
...
static int __exit sunxi_i2s0_dev_remove(struct platform_device *pdev)
{
	if (i2s0_used) {
		...
		if ((NULL == i2s0_pll2clk) ||(IS_ERR(i2s0_pll2clk))) {
			printk("i2s0_pll2clk handle is invalid, just return\n");
			return -EFAULT;
		} else {
			/*release pll2clk*/
			clk_put(i2s0_pll2clk);
		}
