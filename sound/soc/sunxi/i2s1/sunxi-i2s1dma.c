/*
 * sound\soc\sunxi\i2s1\sunxi-i2s1dma.c
...
static int __init sunxi_i2s1_pcm_probe(struct platform_device *pdev)
{
	return snd_soc_register_platform(&pdev->dev, &sunxi_soc_platform);
}

static int __exit sunxi_i2s1_pcm_remove(struct platform_device *pdev)
{
...
static int __init sunxi_soc_platform_i2s1_init(void)
{
	int err = 0;	
	if((err = platform_device_register(&sunxi_i2s1_pcm_device)) < 0)
		return err;

	if ((err = platform_driver_register(&sunxi_i2s1_pcm_driver)) < 0)
		return err;
	return 0;	
}
module_init(sunxi_soc_platform_i2s1_init);

static void __exit sunxi_soc_platform_i2s1_exit(void)
{
	return platform_driver_unregister(&sunxi_i2s1_pcm_driver);
}
module_exit(sunxi_soc_platform_i2s1_exit);

MODULE_AUTHOR("huangxin");
MODULE_DESCRIPTION("SUNXI I2S1 DMA module");
MODULE_LICENSE("GPL");
