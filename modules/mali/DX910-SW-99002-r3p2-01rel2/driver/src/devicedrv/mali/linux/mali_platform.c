...
_mali_osk_errcode_t mali_platform_power_mode_change(mali_power_mode power_mode)
{
    ...
    if(power_mode == MALI_POWER_MODE_ON){
		...
		if(mali_regulator)
		{
			...
			if(mvol != 0)
			{
				if(mvol < mali_dvfs_table[3].vol_max)
					freq = mali_dvfs_table[4].freq_max; 
				else if(mvol < mali_dvfs_table[1].vol_max)
					freq = mali_dvfs_table[3].freq_max; 
				else if(mvol > mali_dvfs_table[1].vol_max + 300*1000*1000)
					freq = mali_dvfs_table[0].freq_max; 
				else
					freq = mali_dvfs_table[1].freq_max;
			}
		}
