...
{
    unsigned char      port;                       //锟剿口猴拷
    unsigned char      port_num;                   //锟剿匡拷锟节憋拷锟
    char      mul_sel;                    //☠锟杰憋拷锟
    char      pull;                       //☠☠状态
    char      drv_level;                  //☠☠☠☠☠☠
    char      data;                       //☠☠锟狡
    unsigned char      reserved[2];                //☠☠位☠☠证☠☠
}
normal_gpio_cfg;
...
typedef struct _boot0_private_head_t
{
	...
	normal_gpio_cfg         uart_ctrl[2];           // UART☠☠☠(☠锟皆达拷印☠)☠☠☠息
	int                     enable_jtag;            // 1 : enable,  0 : disable
    normal_gpio_cfg	        jtag_gpio[5];           // ☠☠JTAG☠全☠GPIO☠息
    normal_gpio_cfg         storage_gpio[32];       // 锟芥储锟借备 GPIO☠息
    char                    storage_data[512 - sizeof(normal_gpio_cfg) * 32];      // 锟矫伙拷☠☠☠☠☠息
