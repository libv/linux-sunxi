...
#if defined(CONFIG_INPUT) && defined (CONFIG_CPU_FREQ_USR_EVNT_NOTIFY)
/*
...
#endif  /* #if defined(CONFIG_INPUT) && defined (CONFIG_CPU_FREQ_USR_EVNT_NOTIFY) */
...
static int __init_vftable_syscfg(void)
{
    ...
    type = script_get_item("dvfs_table", "LV_count", &val);
    ...
    for (i = 1; i <= table_length_syscfg; i++){
        ...
        type = script_get_item("dvfs_table", name, &val);
        if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
    ...
    type = script_get_item("dvfs_table", "max_freq", &val);
    if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
        FANTASY_ERR("get max_freq from sysconfig failed\n");
        goto fail;
    }
    dbs_tuners_ins.pn_perform_freq = val.val / 1000;
