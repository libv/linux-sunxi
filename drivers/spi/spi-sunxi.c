...
int sunxi_spi_dma_init_sg(spi_dma_info_t *info, void *addr, int len, int write)
{
	int i;
	int npages = 0;
	void *bufp = addr;
...
int sunxi_spi_config_dma_rx(struct sunxi_spi *sspi, struct spi_transfer *t)
{
	int ret = 0;
	int nents = 0;
...
int sunxi_spi_config_dma_tx(struct sunxi_spi *sspi, struct spi_transfer *t)
{
	int ret = 0;
	int nents = 0;
	struct dma_slave_config dma_conf = {0};
...
static void sunxi_spi_dma_free_sg(struct sunxi_spi *sspi, spi_dma_info_t *info)
{
	...
	dma_unmap_sg(&sspi->pdev->dev, info->sg, info->nents, info->dir);
...
static int sunxi_spi_clk_init(struct sunxi_spi *sspi, u32 mod_clk)
{
    ...
    sspi->pclk = clk_get(&sspi->pdev->dev, PLL6_CLK);
...
struct platform_device sunxi_spi_device[SUNXI_SPI_NUM];

void __init sunxi_spi_device_scan(void)
{
	int i;
...
int __init sunxi_spi_register_spidev(void)
{
