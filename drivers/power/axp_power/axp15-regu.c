...
static int axp_enable(struct regulator_dev *rdev)
{
	...
	unsigned char devaddr = RSB_RTSADDR_AW1657;
	uint8_t addr = info->enable_reg;
	uint8_t mask = (1 << info->enable_bit);
	uint8_t delay = 0;

	return arisc_axp_set_regs_bits_sync(&devaddr, &addr, &mask, &delay, 1);
...
static int axp_disable(struct regulator_dev *rdev)
{
	...
	return arisc_axp_clr_regs_bits_sync(&devaddr, &addr, &mask, &delay, 1);
...
static struct axp_regulator_info axp_regulator_info[] = {
	AXP15_LDO(1,	700,	3300,	0,	LDO1,	0,	5,	LDO1EN,	5),//ldo1 for aldo1 
	AXP15_LDO(2,	700,	3300,	100,	LDO2,	0,	5,	LDO2EN,	6),//ldo2 for aldo2 
	AXP15_LDO(3,	700,	3300,	100,	LDO3,	0,	5,	LDO3EN,	7),//ldo3 for aldo3 
	AXP15_LDO(4,	700,	1900,	100,	LDO4,	0,	5,	LDO4EN,	0),//ldo3 for bldo1 
	AXP15_LDO(5,	700,	1900,	100,	LDO5,	0,	5,	LDO5EN,	1),//ldo5 for bldo2 
	AXP15_LDO(6,	700,	1900,	100,	LDO6,	0,	5,	LDO6EN,	2),//ldo6 for bldo3 
	AXP15_LDO(7,	700,	1900,	100,	LDO7,	0,	5,	LDO7EN,	3),//ldo7 for bldo4 
	AXP15_LDO(8,	700,	3300,	100,	LDO8,	0,	5,	LDO8EN,	4),//ldo8 for cldo1 
	AXP15_LDO(9,	700,	3400,	100,	LDO9,	0,	5,	LDO9EN,	5),//ldo9 for cldo2
	AXP15_LDO(10,	700,	3300,	100,	LDO10,	0,	5,	LDO10EN,6),//ldo10 for cldo3
	AXP15_DCDC(1,	600,	1100,	10,	DCDC1,	0,	7,	DCDC1EN,0),
	AXP15_DCDC(2,	1000,	2550,	50,	DCDC2,	0,	5,	DCDC2EN,1),
	AXP15_DCDC(3,	600,	1100,	10,	DCDC3,	0,	7,	DCDC3EN,2),
	AXP15_DCDC(4,	600,	1500,	20,	DCDC4,	0,	6,	DCDC4EN,3),
	AXP15_DCDC(5,	1100,	3400,	100,	DCDC5,	0,	5,	DCDC5EN,4),
};
