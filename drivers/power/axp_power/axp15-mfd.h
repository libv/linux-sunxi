...
static int axp15_disable_irqs(struct axp_dev *chip, uint64_t irqs)
{
	...
	ret =  __axp_writes(&devaddr, chip->client, AXP15_INTEN1, 3, v);
...
static int axp15_read_irqs(struct axp_dev *chip, uint64_t *irqs)
{
	...
	int ret;
	unsigned char devaddr = RSB_RTSADDR_AW1657;
	ret =  __axp_reads(&devaddr, chip->client, AXP15_INTSTS1, 2, v);
	if (ret < 0)
		return ret;
...
static int  axp15_init_chip(struct axp_dev *chip)
{
	...
	err =  __axp_read(&devaddr, chip->client, AXP15_IC_TYPE, &chip_id);
	if (err) {
