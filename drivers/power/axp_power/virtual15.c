...
struct device_attribute *axp15_attributes_virtual[] = {
...
static int regulator_virtual_consumer_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(axp15_attributes_virtual); i++) {
		ret = device_create_file(&pdev->dev, axp15_attributes_virtual[i]);
		if (ret != 0)
			goto err;
	}
...
err:
	for (i = 0; i < ARRAY_SIZE(axp15_attributes_virtual); i++)
		device_remove_file(&pdev->dev, axp15_attributes_virtual[i]);
	kfree(drvdata);
