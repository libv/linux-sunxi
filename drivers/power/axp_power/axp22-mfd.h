...
static int axp22_disable_irqs(struct axp_dev *chip, uint64_t irqs)
{
	...
	ret =  __axp_writes(&devaddr, chip->client, AXP22_INTEN1, 9, v);
...
static int axp22_read_irqs(struct axp_dev *chip, uint64_t *irqs)
{
	...
	int ret;
	unsigned char devaddr = RSB_RTSADDR_AW1655;
	ret =  __axp_reads(&devaddr, chip->client, AXP22_INTSTS1, 5, v);
	if (ret < 0)
		return ret;
...
static int  axp22_init_chip(struct axp_dev *chip)
{
	...
	err =  __axp_read(&devaddr, chip->client, AXP22_IC_TYPE, &chip_id);
	if (err) {
	    printk("[AXP22-MFD] try to read chip id failed!\n");
		return err;
	}
	dev_info(chip->dev, "AXP (CHIP ID: 0x%02x) detected\n", chip_id);
	if(((chip_id & 0xc0) == 0x00) && (((chip_id & 0x0f) == 0x06) || ((chip_id & 0x0f) == 0x07)))
		chip->type = AXP22;
	else
		chip->type = AXP22;

	/*enable irqs and clear*/
	err =  __axp_writes(&devaddr, chip->client, AXP22_INTEN1, 19, v);
	if (err) {
