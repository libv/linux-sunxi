...
#if defined CONFIG_ARCH_SUN8IW3 || defined CONFIG_ARCH_SUN9IW1
...
void sunxi_mci_dump_errinfo(struct sunxi_mmc_host* smc_host)
{
	SMC_ERR(smc_host, "smc %d err, cmd %d, %s%s%s%s%s%s%s%s%s%s !!\n",
		...
		dumphex32("mmc",IOMEM(IO_ADDRESS(SMC_BASE(smc_host->pdev->id))),0x100);
}
 ...
 */
static int sunxi_mci_set_clk(struct sunxi_mmc_host* smc_host, u32 clk)
{
	...
	{
#ifndef CONFIG_ARCH_SUN9IW1
		void __iomem *mclk_base = __io_address(0x01c20088 + 0x4 * smc_host->pdev->id);
#else
		void __iomem *mclk_base = __io_address(SUNXI_CCM_BASE + 0x10 + 0x4 * smc_host->pdev->id);
#endif
		SMC_DBG(smc_host,"mclk %x\n",readl(mclk_base));
	}
...
static void sunxi_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->clock && ios->clock != last_clock[id]) {
		...
		#endif
		sunxi_mci_set_clk(smc_host, smc_host->card_clk);
...
static int sunxi_mci_probe(struct platform_device *pdev)
{
	...
	if (smc_host->cd_mode == CARD_DETECT_BY_D3 && smc_host->present == 0) {
		SMC_MSG(smc_host, "sdc%d power init.\n", smc_host->pdev->id);
