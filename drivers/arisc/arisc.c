...
unsigned int arisc_debug_baudrate = 57600;
		...
		struct device_attribute *attr, char *buf)
{
	...
	size = sprintf(buf, "error:%u total count:%u error count:%u\n", arisc_debug_dram_crc_error,
							arisc_debug_dram_crc_total_count, arisc_debug_dram_crc_error_count);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if ((*block_cfg.addr < 0) || (*block_cfg.addr > 0xff)) {
...
static struct device_attribute sunxi_arisc_attrs[] = {
	__ATTR(debug_mask, 	    0644, arisc_debug_mask_show,      arisc_debug_mask_store),
	__ATTR(debug_baudrate,	0644, arisc_debug_baudrate_show,  arisc_debug_baudrate_store),
	__ATTR(dram_crc_paras,	0644, arisc_dram_crc_paras_show,  arisc_dram_crc_paras_store),
	__ATTR(dram_crc_result,	0644, arisc_dram_crc_result_show, arisc_dram_crc_result_store),
#if defined CONFIG_ARCH_SUN8IW1P1 
	__ATTR(p2wi_read_block_data,	0644, arisc_p2wi_read_block_data_show, arisc_p2wi_read_block_data_store),
	__ATTR(p2wi_write_block_data,	0644, arisc_p2wi_write_block_data_show, arisc_p2wi_write_block_data_store),
#elif (defined CONFIG_ARCH_SUN8IW3P1) || (defined CONFIG_ARCH_SUN9IW1P1)
	__ATTR(rsb_read_block_data,	0644, arisc_rsb_read_block_data_show, arisc_rsb_read_block_data_store),
	__ATTR(rsb_write_block_data,	0644, arisc_rsb_write_block_data_show, arisc_rsb_write_block_data_store),
#endif
	__ATTR_NULL,
};
...
#error "please select a platform\n"
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		ARISC_WRN("arisc uart debug config type err!");
		script_val.val = 1;
	}
	if (script_val.val) {
