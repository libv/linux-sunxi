...
void rtl8723b_set_FwRsvdPage_cmd(PADAPTER padapter, PRSVDPAGE_LOC_8723B rsvdpageloc)
{
...
static void rtl8723b_set_FwDisconDecision_cmd(PADAPTER padapter, u8 benable)
{
	...
	u8 adopt = 1, check_period = 100, trypkt_num = 0;
...
static void rtl8723b_set_FwWoWlanCtrl_Cmd(PADAPTER padapter, u8 bFuncEn)
{
	...
	SET_8723B_H2CCMD_WOWLAN_GPIO_PULSE_COUNT(u1H2CWoWlanCtrlParm, 0xff);
...
static void rtl8723b_set_FwRsvdPagePkt(PADAPTER padapter, BOOLEAN bDLFinished)
{
	...
	if (psta == NULL) 
	{
		_rtw_memset(kek, 0, NL80211_KEK_LEN);
		_rtw_memset(kck, 0, NL80211_KCK_LEN);
		DBG_8192C("%s, KEK, KCK download rsvd page all zero \n", __func__);
	}
	else
	{
		_rtw_memcpy(kek, psta->kek, NL80211_KEK_LEN);
		_rtw_memcpy(kck, psta->kck, NL80211_KCK_LEN);
	}
	
	//3(9) KEK, KCK
	RsvdPageLoc.LocGTKInfo = TotalPageNum;
	_rtw_memcpy(ReservedPagePacket+BufIndex-TxDescLen, kck, NL80211_KCK_LEN);
	_rtw_memcpy(ReservedPagePacket+BufIndex-TxDescLen+NL80211_KCK_LEN, kek, NL80211_KEK_LEN);
	...
	//	__FUNCTION__, &ReservedPagePacket[BufIndex-TxDescLen], (TxDescLen + NL80211_KCK_LEN + NL80211_KEK_LEN));

	CurtPktPageNum = (u8)PageNum_128(TxDescLen + NL80211_KCK_LEN + NL80211_KEK_LEN);
