...
BOOLEAN
halbtc8821a1ant_NeedToDecBtPwr(
	IN	PBTC_COEXIST		pBtCoexist
	)
{
	BOOLEAN		bRet=FALSE;
	...
	btRssiState = halbtc8821a1ant_BtRssiState(2, 35, 0);
...
VOID
halbtc8821a1ant_SetFwDacSwingLevel(
	IN	PBTC_COEXIST		pBtCoexist,
	...
	)
{
	...
	halbtc8821a1ant_SetFwDecBtPwr(pBtCoexist, pCoexDm->bCurDecBtPwr);
	...
	)
{
	...
	halbtc8821a1ant_SetFwDacSwingLevel(pBtCoexist, pCoexDm->curFwDacSwingLvl);

	pCoexDm->preFwDacSwingLvl = pCoexDm->curFwDacSwingLvl;
}

VOID
halbtc8821a1ant_SetSwRfRxLpfCorner(
	IN	PBTC_COEXIST		pBtCoexist,
	...
	)
{
	if(bRxRfShrinkOn)
	{
		...
		pBtCoexist->fBtcSetRfReg(pBtCoexist, BTC_RF_A, 0x1e, 0xfffff, 0xffff7);
	}
	else
	{
	...
	)
{
	...
	halbtc8821a1ant_SetSwRfRxLpfCorner(pBtCoexist, pCoexDm->bCurRfRxLpfShrink);
...
VOID
halbtc8821a1ant_SetDacSwingReg(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	u4Byte			level
	)
{
...
VOID
halbtc8821a1ant_SetSwFullTimeDacSwing(
	IN	PBTC_COEXIST		pBtCoexist,
	...
	)
{
	if(bSwDacSwingOn)
	{
		halbtc8821a1ant_SetDacSwingReg(pBtCoexist, swDacSwingLvl);
	}
	else
	{
		halbtc8821a1ant_SetDacSwingReg(pBtCoexist, 0x18);
	}
}


VOID
halbtc8821a1ant_DacSwing(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN			bForceExec,
	...
	)
{
	...
	delay_ms(30);
	halbtc8821a1ant_SetSwFullTimeDacSwing(pBtCoexist, bDacSwingOn, dacSwingLvl);
	...
	)
{
	...
	halbtc8821a1ant_SetAdcBackOff(pBtCoexist, pCoexDm->bCurAdcBackOff);

	pCoexDm->bPreAdcBackOff = pCoexDm->bCurAdcBackOff;
}

VOID
halbtc8821a1ant_SetAgcTable(
	IN	PBTC_COEXIST		pBtCoexist,
	...
	)
{
	...
	halbtc8821a1ant_SetAgcTable(pBtCoexist, bAgcTableEn);
	...
	) 
{
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_BT_MONITOR, ("[BTCoex], SM1[ShRf/ LpRA/ LimDig/ btLna] = %d %d %d %d\n", 
		bShrinkRxLPF, bLowPenaltyRA, bLimitedDIG, bBTLNAConstrain));
	
	//halbtc8821a1ant_RfShrink(pBtCoexist, NORMAL_EXEC, bShrinkRxLPF);
	halbtc8821a1ant_LowPenaltyRa(pBtCoexist, NORMAL_EXEC, bLowPenaltyRA);
}

VOID
halbtc8821a1ant_SwMechanism2(
	IN	PBTC_COEXIST	pBtCoexist,	
	...
	)
{
	...
	//halbtc8821a1ant_AgcTable(pBtCoexist, NORMAL_EXEC, bAGCTableShift);
	//halbtc8821a1ant_AdcBackOff(pBtCoexist, NORMAL_EXEC, bADCBackOff);
	//halbtc8821a1ant_DacSwing(pBtCoexist, NORMAL_EXEC, bSWDACSwing, dacSwingLvl);
}
...
{
	// fw all off
	halbtc8821a1ant_FwDacSwingLvl(pBtCoexist, NORMAL_EXEC, 6);
	halbtc8821a1ant_DecBtPwr(pBtCoexist, NORMAL_EXEC, FALSE);

	// sw all off
	halbtc8821a1ant_SwMechanism1(pBtCoexist,FALSE,FALSE,FALSE,FALSE);
	halbtc8821a1ant_SwMechanism2(pBtCoexist,FALSE,FALSE,FALSE,0x18);
	...
	)
{
	...
	wifiRssiState = halbtc8821a1ant_WifiRssiState(pBtCoexist, 0, 2, 25, 0);

	halbtc8821a1ant_FwDacSwingLvl(pBtCoexist, NORMAL_EXEC, 4);

	if(halbtc8821a1ant_NeedToDecBtPwr(pBtCoexist))
		halbtc8821a1ant_DecBtPwr(pBtCoexist, NORMAL_EXEC, FALSE);
	else	
		halbtc8821a1ant_DecBtPwr(pBtCoexist, NORMAL_EXEC, FALSE);
	...
	if(BTC_WIFI_BW_HT40 == wifiBw)
	{
		...
		if( (wifiRssiState == BTC_RSSI_STATE_HIGH) ||
			(wifiRssiState == BTC_RSSI_STATE_STAY_HIGH) )
		{
			  halbtc8821a1ant_SwMechanism1(pBtCoexist,FALSE,TRUE,FALSE,FALSE);
			  halbtc8821a1ant_SwMechanism2(pBtCoexist,FALSE,FALSE,FALSE,0x18);
		}
		else
		{
	...
	)
{
	...
	wifiRssiState = halbtc8821a1ant_WifiRssiState(pBtCoexist, 0, 2, 25, 0);
	btRssiState = halbtc8821a1ant_BtRssiState(2, 50, 0);
	...
	)
{
	...
	if(BT_8821A_1ANT_BT_STATUS_ACL_BUSY == pCoexDm->btStatus && !pBtCoexist->btLinkInfo.bHidOnly)
		halbtc8821a1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_ON, 0x50, 0x0);
	else
		halbtc8821a1ant_PowerSaveState(pBtCoexist, BTC_PS_WIFI_NATIVE, 0x0, 0x0);
	*/	
	...
	)
{
	...
	BOOLEAN	bLimitedDig=FALSE, bIncreaseScanDevNum=FALSE;
	...
	)
{
	...
	halbtc8821a1ant_FwDacSwingLvl(pBtCoexist, FORCE_EXEC, 6);
	halbtc8821a1ant_DecBtPwr(pBtCoexist, FORCE_EXEC, FALSE);
	...
	)
{
	...
	CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = %d/ %d/ %d ", "SM1[ShRf/ LpRA/ LimDig]", \
			pCoexDm->bCurRfRxLpfShrink, pCoexDm->bCurLowPenaltyRa, pBtCoexist->btInfo.bLimitedDig);
	CL_PRINTF(cliBuf);
...
VOID
EXhalbtc8821a1ant_StackOperationNotify(
	IN	PBTC_COEXIST			pBtCoexist,
	IN	u1Byte				type
	)
{
