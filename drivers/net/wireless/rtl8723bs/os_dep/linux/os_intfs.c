...
void rtw_proc_init_one(struct net_device *dev)
{
	...
	entry = create_proc_read_entry("suspend_info", S_IFREG | S_IRUGO,
				   dir_dev, proc_get_suspend_resume_info, dev);
	if (!entry) {
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	}
	...
	entry = create_proc_read_entry("sd_f0_reg_dump", S_IFREG | S_IRUGO,
				   dir_dev, proc_get_sd_f0_reg_dump, dev);
	if (!entry) {
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	}
	...
	entry = create_proc_read_entry("btcoex", S_IFREG | S_IRUGO,
				   dir_dev, proc_get_btcoex_info, dev);
	if (!entry) {
...
void rtw_proc_remove_one(struct net_device *dev)
{
	...
	if (dir_dev) {
		...
		remove_proc_entry("suspend_info", dir_dev);

		#ifdef CONFIG_SDIO_HCI
		remove_proc_entry("sd_f0_reg_dump", dir_dev);
		#endif
