			...
			union iwreq_data *wdata, char *extra)
{
	...
	else if (strcmp(tmp[0], "btfk2map") == 0)
	{
		...
		{ 
			...
			{ 
				DBG_871X("%s: BT write map afterf compare success\n", __FUNCTION__);
				err = 0;
				goto exit;
			}
			else
			{
			    	  DBG_871X("%s: BT write map afterf compare not match to write efuse \n", __FUNCTION__);
				      if (rtw_BT_efuse_map_write(padapter, 0x00, EFUSE_BT_MAX_MAP_LEN, pEfuseHal->fakeBTEfuseModifiedMap) == _FAIL)
				      {
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	u8 resetbt=0x00;
	u8 H2cSetbtmac[6];
	u8 u1H2CBtMpOperParm[H2C_8723B_BTMP_OPER_LEN]={0x01};
	...
	if ( strncmp(extra, "h2c", 3) == 0){
			...
			FillH2CCmd8723B(padapter, 0x63, 1, u1H2CBtMpOperParm);
			goto exit;
		}
