...
static int test_pin_config_set_api(struct device *dev,
			struct sunxi_pinctrl_test_class *sunxi_pinctrl_test)
{
	int ret=0;
	...
	sunxi_pinctrl_result[2].name = "test_config_set_api";
	test_case_number = 2;
	sunxi_gpio_to_name(sunxi_pinctrl_test->gpio_index, pin_name);
	pr_warn("++++++++++++++++++++++++++++%s++++++++++++++++++++++++++++\n", __func__);
	pr_warn("gpio name: %s	            gpio index: %d\n",pin_name
		,sunxi_pinctrl_test->gpio_index);
	pr_warn("\npin configure we want to set:\n");
	pr_warn("pin func:%d                pin dlevel:%d\n",func,dlevel);
	pr_warn("pin pull:%d                pin data  :%d\n",pul,dat);
	pr_warn("--------------------------------------------\n\n");
	pr_warn("start testing...\n");

	/*check if pin mul setting right */
	pr_warn("1.check [%s] function set api\n",pin_name);
	...
	pr_warn("finish (pin function set) testing.\n");
	pr_warn("-----------------------------------------------\n");
	...
	pr_warn("finish (pin pull set) testing.\n");
	pr_warn("-----------------------------------------------\n");
	...
	pr_warn("finish (pin dlevel set) testing.\n");
	pr_warn("-----------------------------------------------\n");
	
	/*check if pin data setting right */
	pr_warn("4.check [%s] data set api\n",pin_name);
...
static int test_gpio_request_free_api(struct sunxi_pinctrl_test_class *sunxi_pinctrl_test)
{
	...
	char			pin_name[SUNXI_PIN_NAME_MAX_LEN];
	sunxi_pinctrl_result[3].name = "test_gpio_request_free_api";
	test_case_number = 3;
	sunxi_gpio_to_name(sunxi_pinctrl_test->gpio_index, pin_name);
	pr_warn("++++++++++++++++++++++++++++%s++++++++++++++++++++++++++++\n", __func__);
...
static int test_gpio_repeat_request_free_api(struct sunxi_pinctrl_test_class *sunxi_pinctrl_test)
{
	...
	int 			gpio_index = sunxi_pinctrl_test->gpio_index;
	char			pin_name[SUNXI_PIN_NAME_MAX_LEN];
	sunxi_pinctrl_result[4].name="test_gpio_re_request_free_api";
	test_case_number = 4;
	sunxi_gpio_to_name(sunxi_pinctrl_test->gpio_index, pin_name);
	pr_warn("++++++++++++++++++++++++++++%s++++++++++++++++++++++++++++\n", __func__);
	pr_warn("gpio name is : %s	gpio index is : %d\n",pin_name
...
static int test_gpiolib_api(struct sunxi_pinctrl_test_class *sunxi_pinctrl_test)
{
	...
	long unsigned int	config;
	sunxi_pinctrl_result[5].name = "test_gpiolib_api";
	test_case_number = 5;
...
static int test_pinctrl_scripts_api(struct sunxi_pinctrl_test_class *sunxi_pinctrl_test)
{
	...
	sunxi_pinctrl_result[6].name = "test_pinctrl_eint_api";
	test_case_number = 6;
	/*
	[card0_boot_para]
	card_ctrl			= 0
			...
			struct sunxi_pinctrl_test_class *sunxi_pinctrl_test)
{
	...
	int			pin_index=sunxi_pinctrl_test->gpio_index;
	char			pin_name[SUNXI_PIN_NAME_MAX_LEN];
	sunxi_pinctrl_result[8].name="test_pinctrl_re_eint_api";
	test_case_number = 8;
	sunxi_gpio_to_name(pin_index, pin_name);
	pr_warn("++++++++++++++++++++++++++++%s++++++++++++++++++++++++++++\n", __func__);
	pr_warn("gpio name is : %s	gpio index is : %d\n",pin_name,pin_index);
	...
	req_IRQ_status = devm_request_irq(dev, virq, test_sunxi_pinctrl_irq_handler_demo1
	...
	pr_warn("step5:secondary  request irq(low level trigger).\n");
	re_req_IRQ_status = devm_request_irq(dev, virq, test_sunxi_pinctrl_irq_handler_demo2
		,IRQF_TRIGGER_LOW, "PIN_EINT", NULL);
							...
							const char *buf,size_t size)
{
	...
	switch(exec_number){
		...
		case 1:
			...
			break;
		case 2: 
			final=test_pin_config_set_api(dev,sunxi_pinctrl_test);
			break;
		case 3:
