...
static unsigned int devaddr       = RSB_RTSADDR_AW1655;
...
static u64 __arisc_counter_read(void)
{
    ...
    low = readl((const volatile void *)CNT64_CTRL_REG);
    low |= (1<<1);
    writel(low, (volatile void *)CNT64_CTRL_REG);
    while(readl((const volatile void *)CNT64_CTRL_REG) & (1<<1));
	
	low  = readl((const volatile void *)CNT64_LOW_REG);
	high  = readl((const volatile void *)CNT64_HIGH_REG);
	counter = high;
...
int __arisc_dvfs_cb(void *arg)
{
	printk("dvfs fail once\n");
	return 0;
}

void __arisc_dvfs_test(void)
{
	u32 i;
	u32 ret = 0;
	u64 total_time;
...
void __arisc_audio_test(void)
{
    ...
    memset((void *)IO_ADDRESS(AUDIO_SRAM_BASE_PALY), 'a', AUDIO_SRAM_BUF_SIZE_32K);
    memset((void *)IO_ADDRESS(AUDIO_SRAM_BASE_CAPTURE), 'b', AUDIO_SRAM_BUF_SIZE_32K);
    ...
    ret = memcmp((const void *)IO_ADDRESS(AUDIO_SRAM_BASE_PALY), (const void *)IO_ADDRESS(AUDIO_SRAM_BASE_CAPTURE), AUDIO_SRAM_BUF_SIZE_32K);
...
static void __arisc_axp_test(void)
{
	...
	if(arisc_axp_cb_register(__arisc_axp_cb, NULL)) {
...
static void __exit sw_arisc_test_exit(void)
{
	...
	if (test_axp == 1) {
	    ...
	    arisc_axp_cb_unregister(__arisc_axp_cb);
