...
//#define STK_ALLWINNER
//#define STK_ROCKCHIP_PLATFORM
#define STK_ACC_DRIVER_VERSION	"v1.6.2 20130802"
...
// #define GSENSOR_FILTER
#ifdef  GSENSOR_FILTER
#define RANGE_MAX             1
#define RANGE_MIN             -1
#define ZERO_FILER            0 
#define DEFAULT_SENSITIVITY   -21
#endif
...
#define STK831X_INIT_ODR		1		//2:100Hz, 3:50Hz, 4:25Hz
#define STK831X_SAMPLE_TIME_MIN_NO		1
...
#define STK_ACC_CALI_FILE 			"/system/usr/gsensor_offset.txt"
...
static u32 debug_mask = 0x09;
#define dprintk(level_mask, fmt, arg...)	if (unlikely(debug_mask & level_mask)) \
	printk(KERN_DEBUG fmt , ## arg)

module_param_named(debug_mask, debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
#endif /* #ifdef STK_ALLWINNER */
...
static int stk_store_in_file(char offset[], char mode)
{
    ...
    cali_file = filp_open(STK_ACC_CALI_FILE, O_CREAT | O_RDWR,0777);
...
/*	printk(KERN_INFO "%s: cmd = 0x%x\n", __func__, cmd);	*/
	...
	case SENSOR_CALIBRATION:
		...
		STK831x_GetOffset(char3_buffer);
		ret = copy_to_user(argp, &char3_buffer, sizeof(char3_buffer));
		return ret;
	default:
		retval = -ENOTTY;
		break;
...
static int stk831x_irq_setup(struct i2c_client *client, struct stk831x_data *stk_int)
{
	...
	error = input_request_int(&(gsensor_info.input_type), stk_mems_irq_handler, IRQF_TRIGGER_RISING , stk_int);
	if (error < 0) 
	{
...
#define   GSENSOR_DRIVER_VERSION    "<2013/09/22/V1.2>" 
static int stk831x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int error;
	struct stk831x_data *stk;

	printk("Gsensor stk831x_probe: driver version:%s\n",GSENSOR_DRIVER_VERSION);	
	printk("GSENSOR Driver build@%s,%s\n", __TIME__,__DATE__);
	printk("GSENSOR I2C Address:0x%02x\n", client->addr);
	...
	stk->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;	
...
#else	
	input_free_int(&(gsensor_info.input_type), stk);
#if ADDITIONAL_GPIO_CFG 
...
#ifdef CONFIG_HAS_EARLYSUSPEND	
	unregister_early_suspend(&stk->early_suspend);	
#endif
	sysfs_remove_group(&stk->input_dev->dev.kobj, &stk831x_attribute_group);
	misc_deregister(&stk_device);
	input_unregister_device(stk->input_dev);	
...
static void stk831x_early_suspend(struct early_suspend *h)
{
	...
	//struct stk831x_data *stk = i2c_get_clientdata(client);
	struct stk831x_data *stk = container_of(h, struct stk831x_data, early_suspend);
