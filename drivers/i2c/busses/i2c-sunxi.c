...
static void twi_chan_cfg(void)
{
	...
	for (i=0; i<SUNXI_TWI_NUM; i++) {
		...
		if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
			I2C_ERR("[twi%d] fetch para from sysconfig failed\n", i);
			continue;
		}
...
*
*  Description:
*            ☠☠slave☠址☠7bit☠全☠☠息☠☠10bit锟侥碉拷一☠锟街碉拷址☠☠锟解部锟接口碉拷锟矫ｏ拷锟节诧拷实锟街★拷
...
void sunxi_i2c_release(struct device *dev)
{
	I2C_ENTER();
}
...
struct platform_device sunxi_twi_device[SUNXI_TWI_NUM];

void sunxi_twi_device_scan(void)
{
	int i;
