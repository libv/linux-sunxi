...
static unsigned int dramfreq_usb_lock = 0;
static unsigned int dramfreq_wake_lock = 1;
static unsigned int dramfreq_wake_lock_soft = 1;
...
{
    while (1) {
        if (dramfreq_wake_lock) {
            mod_timer(&dramfreq_timer, jiffies + msecs_to_jiffies(1000));
            return;
        } else {
            dramfreq_wake_lock_soft = 0;
            break;
        }
    }
...
{
    if (dramfreq_usb_lock) {
        VANS_DBG("usb already in, do not need early suspend\n");
        return;
    }
...
static void dramfreq_late_resume(struct early_suspend *h)
{
    ...
    if (dramfreq_usb_lock) {
        VANS_DBG("usb already in, do not need late resume\n");
        return;
    }
    ...
    if (time_before(jiffies, dramfreq_expired_time)) {
        ...
        if (dramfreq_wake_lock_soft) {
            VANS_DBG("exist other wake lock, do not need late resume\n");
            del_timer_sync(&dramfreq_timer);
            mutex_unlock(&timer_mutex);
            goto out;
        }

        VANS_DBG("%s:%s schedule work\n", __FILE__, __func__);
        del_timer_sync(&dramfreq_timer);
        schedule_work(&dramfreq_work);
    }
    mutex_unlock(&timer_mutex);
...
static ssize_t show_wake_lock(struct device *dev, struct device_attribute *attr,
            char *buf)
{
    return sprintf(buf, "%u\n", dramfreq_wake_lock);
}
            ...
            const char *buf, size_t count)
{
    ...
    mutex_lock(&df->lock);
    dramfreq_usb_lock = value;
    if (dramfreq_usb_lock == 1) {
        dramfreq_need_suspend = 0;
        del_timer_sync(&dramfreq_timer);
        df->scaling_min_freq = df->max_freq;
    } else if(dramfreq_usb_lock == 0) {
...
static ssize_t store_wake_lock(struct device *dev, struct device_attribute *attr,
            const char *buf, size_t count)
{
    struct devfreq *df = to_devfreq(dev);
    unsigned int value;
    ...
    mutex_lock(&df->lock);
    dramfreq_wake_lock = value;
    if ((dramfreq_wake_lock != 0) && (dramfreq_wake_lock != 1)) {
        VANS_ERR("unsupported paras: %u\n", dramfreq_wake_lock);
        dramfreq_wake_lock = 1;
        mutex_unlock(&df->lock);
        goto out;
    }
...
static DEVICE_ATTR(wake_lock, 0600, show_wake_lock, store_wake_lock);
