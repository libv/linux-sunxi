...
struct testclk_data testclk_priv;
void clktest_reg_dump(void)
{
	unsigned int i;
	char dumpline[256];
	printk("Dump Base 0x%x Regs %d as Follow:\n",(unsigned int)sunxi_clk_base, (unsigned int)sunxi_clk_maxreg);
	for(i=0;i<=sunxi_clk_maxreg;)
	{
			if(i+12 <= sunxi_clk_maxreg) 
			sprintf(dumpline,"[Reg %08x] %08x %08x %08x %08x ",i
						,*((unsigned int*)((unsigned int)(sunxi_clk_base +i)))
						,*((unsigned int*)((unsigned int)(sunxi_clk_base +i+4)))
						,*((unsigned int*)((unsigned int)(sunxi_clk_base +i+8)))
						,*((unsigned int*)((unsigned int)(sunxi_clk_base +i+12))));
    ...
    if(cpus_clk_maxreg)
    {
        printk("Dump CPUS Base 0x%x Regs %d as Follow:\n",(unsigned int)sunxi_clk_cpus_base,(unsigned int)cpus_clk_maxreg);
        for(i=0;i<=cpus_clk_maxreg;)
        {
                if(i+12 <= cpus_clk_maxreg) 
                sprintf(dumpline,"[Reg %08x] %08x %08x %08x %08x ",i
                            ,*((unsigned int*)((unsigned int)(sunxi_clk_cpus_base +i)))
                            ,*((unsigned int*)((unsigned int)(sunxi_clk_cpus_base +i+4)))
                            ,*((unsigned int*)((unsigned int)(sunxi_clk_cpus_base +i+8)))
                            ,*((unsigned int*)((unsigned int)(sunxi_clk_cpus_base +i+12))));
