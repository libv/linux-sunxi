...
static int wdt_set_tmout(int timeout_in_sec)
{
	...
	WD_DBG("%s, write 0x%08x to mode reg 0x%08x, actual timeout %d sec\n",
...
static int sunxi_wdt_release(struct inode *inode, struct file *file)
{
	if(expect_release)
		...
		WD_DBG("%s: unexpected close, not stopping watchdog!\n", __func__);
		watchdog_kick(); 
	}
...
static int  sunxi_wdt_probe(struct platform_device *pdev)
{
	...
	if(!g_timeout || g_timeout > MAX_TIMEOUT) {
		...
		printk(KERN_INFO PFX "timeout value invalid, using %d\n", g_timeout);
	}
	...
	WD_DBG("%s: devm_ioremap return wdt_reg 0x%08x, res->start 0x%08x, res->end 0x%08x",
	...
	printk(KERN_INFO PFX "initialized (g_timeout=%ds, g_nowayout=%d)\n",
