...
static __s32 sunxi_hcd_bsp_init(__u32 usbc_no, sunxi_hcd_io_t *sunxi_hcd_io)
{
	...
	sunxi_hcd_io->usbc.usbc_info[usbc_no].base = (u32)sunxi_hcd_io->usb_vbase;
	sunxi_hcd_io->usbc.sram_base = (u32)sunxi_hcd_io->sram_vbase;
...
//	USBC_init(&sunxi_hcd_io->usbc);
	...
	clear_usb_reg((u32)sunxi_hcd_io->usb_vbase);
	fpga_config_use_otg((u32)sunxi_hcd_io->sram_vbase);
#endif
    return 0;
}
...
//	DMSG_INFO_HCD0("[usb host]: sram_vbase   = 0x%x\n", (u32)sunxi_hcd_io->sram_vbase);
	...
	USBC_ConfigFIFO_Base(sunxi_hcd_io->usb_bsp_hdle, (u32)sunxi_hcd_io->sram_vbase, USBC_FIFO_MODE_8K);
	...
	DMSG_INFO("[sunxi_hcd0]: host_init_state = %d\n", sunxi_hcd_io->host_init_state);
...
io_failed:
	sunxi_hcd_io->usb_vbase = 0;
	sunxi_hcd_io->sram_vbase = 0;
...
static void sunxi_hcd_shutdown(struct platform_device *pdev)
{
    ...
    if(sunxi_hcd == NULL){
        DMSG_INFO("err: sunxi_hcd is null\n");
    	return ;
    }
...
*******************************************************************************
*/
static int fifo_setup(struct sunxi_hcd *sunxi_hcd,
...
static int ep_config_from_table(struct sunxi_hcd *sunxi_hcd)
{
	...
	DMSG_DBG_HCD("ep_config_from_table: %s: %d/%d max ep, %d/%d memory\n",
			     ...
			     n + 1, sunxi_hcd->config->num_eps * 2 - 1,
...
static int ep_config_from_hw(struct sunxi_hcd *sunxi_hcd)
{
	...
	void                *usbc_base  = sunxi_hcd->mregs;
	int                 ret         = 0;
...
{
	struct sunxi_hcd *sunxi_hcd = container_of(data, struct sunxi_hcd, irq_work);
       void __iomem *usbc_base = sunxi_hcd->mregs;
       int reg_val = 0;
...
int sunxi_usb_disable_hcd0(void)
{
	...
	DMSG_INFO("sunxi_usb_disable_hcd0 start, clk_is_open = %d\n",
	...
	DMSG_INFO("sunxi_usb_disable_hcd0 end\n");
#endif
...
int sunxi_usb_enable_hcd0(void)
{
	...
	DMSG_INFO("sunxi_usb_enable_hcd0 start, clk_is_open = %d\n",
		      		sunxi_hcd->sunxi_hcd_io->clk_is_open);
	...
	DMSG_INFO("sunxi_usb_enable_hcd0 end\n");
#endif
...
static int sunxi_hcd_suspend(struct device *dev)
{
	...
	if(!sunxi_hcd->sunxi_hcd_io->clk_is_open){
		DMSG_INFO("wrn: sunxi_hcd_suspend, usb clock is close, can't close again\n");
		return 0;
	}
...
{	struct platform_device *pdev = to_platform_device(dev);
	...
	if(sunxi_hcd->sunxi_hcd_io->clk_is_open){
		DMSG_INFO("wrn: sunxi_hcd_suspend, usb clock is open, can't open again\n");
		return 0;
	}
