...
/*
*******************************************************************************
*                     sunxi_hcd_clean_dma_status
*
* Description:
*    ☠锟ep☠☠DMA☠☠☠状态, 一☠☠DMA锟届常☠时☠☠锟
*
* Parameters:
*    qh  :  input.
*
...
/*
*******************************************************************************
*              sunxi_hcd_clean_ep_dma_status_and_flush_fifo
*
* Description:
*    ☠锟ep☠☠DMA☠☠☠状态, 一☠☠DMA锟届常☠时☠☠锟
*
...
*******************************************************************************
*/
void sunxi_hcd_clean_ep_dma_status_and_flush_fifo(struct sunxi_hcd_qh *qh)
{
	struct sunxi_hcd_hw_ep   *hw_ep = NULL;
    u8  	old_ep_index 	= 0;
	__hdle 	usb_bsp_hdle 	= 0;
                                ...
                                u32 length)
{
	...
	sunxi_hcd_dma_start(qh, (u32)hw_ep->fifo, (dma_addr_t)urb->transfer_buffer + offset, length);
                                  ...
                                  int is_in)
{
	...
	if (!is_host_active(sunxi_hcd) || !sunxi_hcd->is_active){
	    DMSG_PANIC("ERR: sunxi_hcd_advance_schedule, host is not active\n");
		return ;
    }
    if (qh != NULL) {
		if(qh->is_ready){
                            ...
                            u32 len)
{
	...
	if (is_out) {   /* tx */
		...
		if(is_sunxi_hcd_dma_capable(sunxi_hcd->usbc_no, len, packet_sz, epnum)){
			...
			sunxi_hcd_dma_start(qh, (u32)hw_ep->fifo, (dma_addr_t)(urb->transfer_buffer + offset), len);
...
irqreturn_t sunxi_hcd_h_ep0_irq(struct sunxi_hcd *sunxi_hcd)
{
	...
	urb = next_urb(qh);
	if(urb == NULL){
		DMSG_PANIC("ERR: sunxi_hcd_h_ep0_irq, urb is NULL\n");
		return IRQ_HANDLED;
	}
	...
	if (csr & (1 << USBC_BP_CSR0_H_RxStall)) {
		DMSG_PANIC("ERR: sunxi_hcd_h_ep0_irq, STALLING ENDPOINT\n");
	...
	if (status) {
		DMSG_PANIC("ERR: sunxi_hcd_h_ep0_irq, ep aborting\n");
...
void sunxi_hcd_host_tx(struct sunxi_hcd *sunxi_hcd, u8 epnum)
{
	...
	if (is_sunxi_hcd_dma_capable(sunxi_hcd->usbc_no, qh->segsize, qh->maxpacket, epnum)
		&& !status) {
		/* dma锟叫断诧拷☠锟皆猴拷USB锟借备☠☠☠☠取☠FIFO锟叫碉拷☠☠,
		 * ☠☠☠TX_READY☠一直☠☠锟捷憋拷取☠为止☠
		 *
		 * ☠☠☠☠锟, dma☠☠☠☠☠☠院锟, ☠callback, ☠☠直☠return☠
		 */
...
void sunxi_hcd_host_rx(struct sunxi_hcd *sunxi_hcd, u8 epnum)
{
	...
	urb = next_urb(qh);
	if(urb == NULL){
		DMSG_PANIC("ERR: sunxi_hcd_host_rx, urb is NULL\n");
		return ;
	}
	...
	if (is_sunxi_hcd_dma_capable(sunxi_hcd->usbc_no, qh->segsize, qh->maxpacket, epnum) && (rx_csr & (1 << USBC_BP_RXCSR_H_DMA_REQ_EN))) {
		...
		if (unlikely(!(rx_csr & (1 << USBC_BP_RXCSR_H_RX_PKT_READY)))) {
			...
			DMSG_PANIC("ERR: sunxi_hcd_host_rx, Rx interrupt with no errors or packet!\n");
		...
		if (is_sunxi_hcd_dma_capable(sunxi_hcd->usbc_no, qh->segsize, qh->maxpacket, epnum)) {
			...
			sunxi_hcd_dma_start(qh, (u32)hw_ep->fifo, buf, length);
...
int sunxi_hcd_urb_enqueue(struct usb_hcd *hcd, struct urb *urb, gfp_t mem_flags)
{
	...
	if (qh->maxpacket & ~0x7ff) {
	    DMSG_PANIC("ERR: sunxi_hcd_urb_enqueue, ep packet is too big, maxpacket = %d\n", qh->maxpacket);
		ret = -EMSGSIZE;
		goto done;
	}
