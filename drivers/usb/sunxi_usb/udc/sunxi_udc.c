...
static int dma_write_fifo(struct sunxi_udc_ep *ep, struct sunxi_udc_request *req)
{
	...
	sunxi_udc_dma_start(ep, fifo_reg, req->req.dma, left_len);
	spin_lock(&ep->dev->lock);
...
DMSG_DBG_UDC("sunxi_udc_handle_ep0--3--%d\n", dev->ep0state);
	...
	ep0csr = USBC_Readw(USBC_REG_RXCSR(g_sunxi_udc_io.usb_bsp_hdle));
...
*******************************************************************************
*/
void sunxi_udc_clean_dma_status(struct sunxi_udc_ep *ep)
{
    u8 ep_index = 0;
	u8 old_ep_index = 0;
	struct sunxi_udc_request	*req = NULL;
...
void sunxi_udc_dma_completion(struct sunxi_udc *dev, struct sunxi_udc_ep *ep, struct sunxi_udc_request *req)
{
	...
	if(req->req.length > req->req.actual){
		if(((ep->bEndpointAddress & USB_DIR_IN) != 0)
			&& !USBC_Dev_IsWriteDataReady(dev->sunxi_udc_io->usb_bsp_hdle, USBC_EP_TYPE_TX)){
				 ...
				 const struct usb_endpoint_descriptor *desc)
{
	...
	if ((ep->bEndpointAddress) & USB_DIR_IN){ /* tx */
	    USBC_Dev_ConfigEp(g_sunxi_udc_io.usb_bsp_hdle, USBC_TS_TYPE_BULK, USBC_EP_TYPE_TX, SW_UDC_FIFO_NUM, _ep->maxpacket & 0x7ff);
    	USBC_ConfigFifo(g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_TX, SW_UDC_FIFO_NUM, 512, fifo_addr);

		//开启该ep的tx_irq en
		USBC_INT_EnableEp(g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_TX, ep->num);
	}else{	 /* rx */
	    USBC_Dev_ConfigEp(g_sunxi_udc_io.usb_bsp_hdle, USBC_TS_TYPE_BULK, USBC_EP_TYPE_RX, SW_UDC_FIFO_NUM, _ep->maxpacket & 0x7ff);
   		USBC_ConfigFifo(g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_RX, SW_UDC_FIFO_NUM, 512, fifo_addr);

		//开启该ep的rx_irq
		USBC_INT_EnableEp(g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_RX, ep->num);
	}
...
s32  usbd_stop_work(void)
{
    ...
    USBC_Dev_ConectSwitch(g_sunxi_udc_io.usb_bsp_hdle, USBC_DEVICE_SWITCH_OFF);	//Ĭ��Ϊpulldown
...
*******************************************************************************
*/
int sunxi_udc_start(struct usb_gadget_driver *driver,
		                    int (*bind)(struct usb_gadget *))
{
	...
	/*linux 3.0��3.3�����ݽṹusb_gadget_driver��С�Ķ�speed->max_spee */
...
*******************************************************************************
*/
int sunxi_udc_stop(struct usb_gadget_driver *driver)
{
	struct sunxi_udc *udc = the_controller;
...
static struct platform_driver sunxi_udc_driver = {
	...
	.remove		= __devexit_p(sunxi_udc_remove), //__exit_p
