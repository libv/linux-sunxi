...
static __s32 usb_script_parse(struct usb_cfg *cfg)
{
	...
	for(i = 0; i < cfg->usbc_num; i++){
		...
		if(type == SCIRPT_ITEM_VALUE_TYPE_PIO){
			...
			DMSG_INFO("get usbc(%d) det_vbus gpio failed and try to axp det_pin\n", i);
		}
...
static __s32 check_usb_board_info(struct usb_cfg *cfg)
{
    ...
    if(cfg->port[0].enable){
        ...
        /* ☠☠锟VBUS/ID☠夥绞斤拷☠捅☠☠锟id/vbus pin ☠☠效☠ */
...
static int __init usb_manager_init(void)
{
	...
	usbc.usbc_info[0].base = SUNXI_USB_OTG_VBASE;
	usbc.sram_base = SUNXI_SRAMCTRL_VBASE;
	USBC_init(&usbc);
