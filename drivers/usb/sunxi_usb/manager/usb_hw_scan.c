...
static __u32 PIODataIn_debounce(struct usb_gpio *usb_gpio, __u32 *value)
{
	...
	__u32 temp1  = 0;
	__u32 cnt    = 0;
	__u32 change = 0;	/* 锟角凤拷锟叫讹拷☠? */

    /* 取 10 ☠PIO☠状态☠☠锟10锟轿碉拷值☠一☠☠说☠☠锟轿讹拷☠☠☠效☠
       ☠☠☠为☠锟轿讹拷☠☠失锟杰★拷
    */
    if(usb_gpio->valid){
        retry = time;
			...
			switch(port_info->detect_type){
				...
				case USB_DETECT_TYPE_VBUS_ID:
				{
					...
					if(port_info->id.valid){
						...
						/* reserved is pull up */
						if(need_pull_pio){
							//gpio_direction_output(port_info->id.gpio_set.gpio.gpio, 1);
