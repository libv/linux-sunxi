...
static int sunxi_ohci_hcd_probe(struct platform_device *pdev)
{
	...
	if(!hcd){
  	...
  	hcd->rsrc_start = (u32)sunxi_ohci->ohci_base;
...
*******************************************************************************
*/
void sunxi_ohci_hcd_shutdown(struct platform_device* pdev)
{
	struct sunxi_hci_hcd *sunxi_ohci = NULL;
