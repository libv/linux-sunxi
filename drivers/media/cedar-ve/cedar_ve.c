...
{	
	...
	/*☠☠run_task_list☠☠☠☠☠☠☠☠☠☠燃☠☠☠锟节碉拷锟叫碉拷☠☠☠锟饺硷拷锟竭ｏ拷☠锟揭碉拷前☠☠☠☠锟角碉拷一☠☠☠☠☠锟
	 *☠么锟酵斤拷☠锟饺硷拷锟竭碉拷☠☠☠锟角帮拷妫拷☠☠械☠☠☠取锟接高碉拷锟阶碉拷☠锟饺硷拷☠☠锟脚队★拷
	 */
	...
	printk("\n");
	#endif
	/*每锟轿诧拷☠一☠☠锟今，就斤拷☠前锟侥硷拷时☠时☠☠☠为系统☠前☠jiffies*/	
...
int cedardev_del_task(int task_id)
{
	...
	/*☠☠run_task_list☠☠
	*☠☠业☠锟接拷锟id锟脚ｏ拷☠么锟酵斤拷run_task_list☠☠锟叫碉拷☠☠锟狡碉拷del_task_list☠☠谋锟酵凤拷锟
	*/
	list_for_each_entry(task_entry, &run_task_list, list) {
...
static void cedar_engine_for_events(unsigned long arg)
{
	...
	list_for_each_entry_safe(task_entry, task_entry_tmp, &del_task_list, list) {		
		...
		if (task_entry->status == TASK_TIMEOUT){//☠示☠☠timeout删☠
			info.si_errno = TASK_TIMEOUT;			
			send_sig_info(SIG_CEDAR, &info, task_entry->task_handle);
		}else if(task_entry->status == TASK_RELEASE){//☠示☠☠☠☠☠☠☠锟缴撅拷锟
	...
	if(!list_empty(&run_task_list)){
		...
		if(task_entry->running == 0){
			...
			info.si_signo = SIG_CEDAR;
			info.si_code = task_entry->t.ID;
			info.si_errno = TASK_INIT;	//☠☠锟窖撅拷☠锟
			send_sig_info(SIG_CEDAR, &info, task_entry->task_handle);
		}
	...
	switch (cmd)
	{
   		case IOCTL_ENGINE_REQ:
			...
			if(!list_empty(&run_task_list) && ( task_ret.block_mode == CEDAR_NONBLOCK_TASK)){
				spin_unlock_irqrestore(&cedar_spin_lock, flags);
				return CEDAR_RUN_LIST_NONULL; //run_task_list☠☠☠☠锟今，凤拷☠-1
			}
			spin_unlock_irqrestore(&cedar_spin_lock, flags);
			
			/*☠锟task为☠☠状态☠☠task☠☠run_task_list☠☠☠*/			
			...
			return task_ptr->is_first_task;//☠☠run_task_list☠☠锟叫碉拷☠☠锟角碉拷一☠☠锟今，凤拷☠1☠☠锟角碉拷一☠☠锟今返伙拷0. hx modify 2011-7-28 16:59:16☠☠☠			
		#else
			enable_cedar_hw_clk();
    	...
    	case IOCTL_ENGINE_REL:
			...
			rel_taskid = (int)arg;		
			/*
			*	☠☠☠☠锟id锟脚斤拷☠☠☠锟缴撅拷☠☠☠☠☠锟街碉拷☠澹猴拷也☠☠锟接ID☠☠☠-1;锟揭碉拷☠应ID☠☠☠0☠
			*/			
		...
		case IOCTL_ENGINE_CHECK_DELAY:		
			{
	            ...
	            /*☠锟矫伙拷锟秸硷拷锟叫伙拷取要☠询☠☠☠☠锟饺硷拷☠通☠☠☠☠锟饺硷拷☠统☠☠要锟饺达拷☠☠时☠total_time.
				* ☠☠☠涌☠校锟酵币诧拷☠没☠☠☠说锟角帮拷☠☠frametime☠☠☠☠☠锟皆硷拷锟劫接口ｏ拷☠☠锟矫伙拷锟秸硷拷要☠☠☠一☠锟秸碉拷frametime值☠
				*☠锟节碉拷前task☠frametime☠也☠☠锟矫讹拷☠慕涌诨锟饺★拷☠☠☠☠☠☠锟frametime☠total_time锟酵达拷锟节诧拷同锟接匡拷锟叫★拷锟矫达拷☠☠☠
				*/
				...
				task_info.total_time = cedardev_check_delay(task_info.task_prio);//task_info.task_prio锟角达拷锟捷癸拷☠☠☠锟饺硷拷
				#ifdef CEDAR_DEBUG
				...
				spin_lock_irqsave(&cedar_spin_lock, flags);
				if(!list_empty(&run_task_list)){
					/*☠取run_task_list☠☠锟叫的碉拷一☠☠☠也☠锟角碉拷前☠锟叫碉拷☠☠通☠☠前☠锟叫碉拷☠☠锟饺frametime时☠*/					
					struct cedarv_engine_task *task_entry;
					#ifdef CEDAR_DEBUG
					printk("%s,%d\n",__func__,__LINE__);
					#endif
				...
				spin_unlock_irqrestore(&cedar_spin_lock, flags);
				/*
				*☠☠☠☠锟饺硷拷☠total_time,frametime☠☠☠锟矫伙拷锟秸间。☠☠☠锟饺硷拷☠☠锟矫伙拷☠锟矫碉拷值☠total_time☠☠要锟饺达拷☠☠时锟戒，
				*frametime锟角碉拷前☠☠☠☠锟绞憋拷锟.☠实☠前☠☠☠锟较拷☠☠☠一☠锟接匡拷实☠.☠☠☠隙群徒涌诘☠锟秸癸拷锟.
				*/
...
//  cedar_devp->iomap_addrs.regs_avs = ioremap(AVS_REGS_BASE, 1024);
	...
	/*☠cedar drv☠始☠☠时锟津，筹拷始☠☠时☠☠☠☠☠锟侥筹拷员
	* ☠☠☠☠☠锟run_task_list☠时☠☠☠锟绞憋拷☠☠☠☠枚锟绞憋拷☠锟绞憋拷锟轿拷锟角跋低筹拷锟jiffies☠锟轿匡拷cedardev_insert_task
	*/
