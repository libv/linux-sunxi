...
static int sensor_s_fmt_before(struct v412_subdev *sd)
{
	...
	ret = sensor_write(sd,0x0300, 0xc1);

	ret = sensor_read(sd, 0x0012, &g_pv_shutter_reg12);
	printk("snesor_s_fmt_before g_pv_shutter_reg12 = [%x]\n", g_pv_shutter_reg12);

	ret = sensor_read(sd, 0x0013, &g_pv_shutter_reg13);
	printk("snesor_s_fmt_before g_pv_shutter_reg13 = [%x]\n", g_pv_shutter_reg13);
	

	ret = sensor_read(sd, 0x0014, &AGain_shutter_reg14);
	   

	ret = sensor_read(sd, 0x0015, &AGain_shutter_reg15);
	   

	ret = sensor_read(sd, 0x0016, &DGain_shutter_reg16);
	

	ret = sensor_read(sd, 0x0017, &DGain_shutter_reg17);	 
...
static int sensor_s_fmt_after(struct v412_subdev *sd)
{
	...
	ret = sensor_write(sd, 0x0304, g_pv_shutter_reg12);


	ret = sensor_write(sd, 0x0305, g_pv_shutter_reg13);


	ret = sensor_write(sd, 0x0307, AGain_shutter_reg15);

	ret = sensor_write(sd, 0x0306, AGain_shutter_reg14);

	ret = sensor_write(sd, 0x0308, ((((DGain_shutter_reg16 << 8) | (DGain_shutter_reg17 & 0xFF)) >> 2) & 0xFF));
	
	ret = sensor_write(sd, 0x0300, 0x01);
	msleep(50);
	return 0;
}
/*
             ...
             struct v4l2_mbus_framefmt *fmt)//linux-3.0
{
	...
	if (wsize->width == 1600 && wsize->height == 1200)
	 {
		sensor_s_fmt_after(sd);
		msleep(300);
	}
		if (wsize->width == 800 && wsize->height == 600)
	 {
