...
static char *           dev_name        = "/dev/video0";//☠☠头锟借备☠
static int              fd              = -1;
...
static int read_frame (void)
{
	...
	ioctl (fd, VIDIOC_DQBUF, &buf); //☠锟叫采硷拷☠帧☠☠
...
//	printf ("buf.m.offset = 0x%x\n",buf.m.offset);		
	...
	if(count == 60)
    {
        ...
        if(buf_size_1 != 0)
        {
		    file_fd = fopen("/mnt/fb1.bin","w");    
		    fwrite(buffers[buf.index].start, buf_size_1, 1, file_fd); //陆芦扦写色录锟   fclose(file_fd);
		}
        if(buf_size_2 != 0)
        {
		    file_fd = fopen("/mnt/fb2.bin","w");
		    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1), buf_size_2, 1, file_fd); //陆芦扦写色录锟   fclose(file_fd);
	    }
        if(buf_size_3 != 0)
        {
		    file_fd = fopen("/mnt/fb3.bin","w");
		    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1+buf_size_2), buf_size_3, 1, file_fd); //陆芦扦写色录锟}
	    }
	}
...
int main_test (void)
{
	...
	//fd = open (dev_name, O_RDWR /* required */ | O_NONBLOCK, 0);//☠锟借备
	//fd = open (dev_name, O_RDWR /* required */ | O_NONBLOCK, 0);//☠锟借备
	//close (fd);
	fd = open (dev_name, O_RDWR /* required */ | O_NONBLOCK, 0);//☠锟借备
	...
	if(ioctl_test==1)
	{
			...
			if (-1 == ioctl (fd, VIDIOC_QUERYCAP, &cap))//☠取☠☠头☠☠
		...
		//Test VIDIOC_ENUMINPUT,VIDIOC_S_INPUT,VIDIOC_G_INPUT
			...
			{
				inp.index = i;
				if (-1 == ioctl (fd, VIDIOC_ENUMINPUT, &inp))//☠取☠☠☠锟
				...
				if (-1 == ioctl (fd, VIDIOC_G_INPUT, &inp))	//☠取☠☠index
		...
		//Test VIDIOC_ENUM_FMT
			...
			{
				fmtdesc.index = i;
				if (-1 == ioctl (fd, VIDIOC_ENUM_FMT, &fmtdesc))//☠取☠式☠☠
					{
...
//		printf("%s %d\n",__FILE__,__LINE__);
		//Test VIDIOC_S_FMT
            ...
            subch_size.width = 640;
            subch_size.height = 480;
			...
			*/
			...
			if(invalid_fmt_test==1)
			{
				...
				if (-1 == ioctl (fd, VIDIOC_S_FMT, &fmt)) //☠锟矫达拷☠图☠锟绞
				{
	...
	if (-1 == ioctl (fd, VIDIOC_S_PARM, &parms)) //☠取帧☠
	...
	if(fps_test==1)
	{
		//Test VIDIOC_G_PARM
			...
			if (-1 == ioctl (fd, VIDIOC_G_PARM, &parms)) //☠取帧☠
...
//			printf("denominator = %d\n",parms.parm.capture.timeperframe.denominator);	
			...
			if (-1 == ioctl (fd, VIDIOC_S_FMT, &fmt)) //☠☠图☠锟绞
			{
					printf("VIDIOC_S_FMT error!\n");
					ret = -1;
//					return ret;
...
//		printf("%s %d\n",__FILE__,__LINE__);
		...
		//Test VIDIOC_G_FMT	
		if (-1 == ioctl (fd, VIDIOC_G_FMT, &fmt)) //☠取图☠锟绞
		{
	...
	ioctl (fd, VIDIOC_REQBUFS, &req); //☠锟诫缓锟藉，count☠☠☠☠☠锟

	buffers = calloc (req.count, sizeof (*buffers));//锟节达拷锟叫斤拷☠☠应锟秸硷拷

	for (n_buffers = 0; n_buffers < req.count; ++n_buffers) 
	{
	   struct v4l2_buffer buf;   //☠☠锟叫碉拷一帧
	   CLEAR (buf);
	   buf.type        = V4L2_BUF_TYPE_VIDEO_CAPTURE;
	   buf.memory      = V4L2_MEMORY_MMAP;
	   buf.index       = n_buffers;

	   if (-1 == ioctl (fd, VIDIOC_QUERYBUF, &buf)) //映☠锟矫伙拷锟秸硷拷
			printf ("VIDIOC_QUERYBUF error\n");

	   buffers[n_buffers].length = buf.length;
	   buffers[n_buffers].start  = mmap (NULL /* start anywhere */,    //通☠mmap☠☠映☠锟较
								         buf.length,
	...
	for (i = 0; i < n_buffers; ++i) 
	{
	   ...
	   if (-1 == ioctl (fd, VIDIOC_QBUF, &buf))//☠锟诫到锟侥伙拷☠☠☠卸锟
	...
	if (-1 == ioctl (fd, VIDIOC_STREAMON, &type)) //☠始☠捉图☠☠☠
...
//	while(1)
	{
		...
		for (;;) //☠一☠锟芥及☠锟届步IO
		{
		   ...
		   FD_ZERO (&fds);//☠指☠☠锟侥硷拷☠☠☠☠☠锟
		   FD_SET (fd, &fds);//☠锟侥硷拷☠☠☠☠☠☠☠☠一☠锟铰碉拷锟侥硷拷☠☠☠
		
		   /* Timeout. */
		   tv.tv_sec = 2;
		   tv.tv_usec = 0;
		
		   r = select (fd + 1, &fds, NULL, NULL, &tv);//锟叫讹拷锟角凤拷啥☠☠☠☠锟酵凤拷欠锟阶硷拷☠茫☠锟tv锟角讹拷时
      ...
      if(count==read_num-1)
      	disp_on();                  
#endif		
		   if (read_frame ())//☠☠啥☠锟街达拷锟read_frame ()☠☠☠☠☠☠循☠
		   break;
		} 
	}
...
close:	
	if (-1 == ioctl (fd, VIDIOC_STREAMOFF, &type)) //停止☠捉图☠☠☠
...
main(void)
{
        ...
        input_size.width = 640;//640;//1280;//1600;//640;
...
//		disp_size.height = 1024;//1200;//480;
		csi_format = V4L2_PIX_FMT_NV21;//V4L2_PIX_FMT_YUV444;//V4L2_PIX_FMT_NV16;//V4L2_PIX_FMT_NV12;//V4L2_PIX_FMT_NV21;
		csi_field = V4L2_FIELD_NONE;//V4L2_FIELD_INTERLACED;//V4L2_FIELD_NONE;
		
		subch_format = V4L2_PIX_FMT_NV21;
