...
#define VFE_MODULE_NAME "sunxi_vip"
//#define REG_DBG_EN
  ...
  for(i=0; i < dev->dev_qty; i++) {
    if(dev->ccm_cfg[i]->is_isp_used && dev->ccm_cfg[i]->is_bayer_raw) {
      ...
      if(!ret) {
        ...
        dev->isp_tbl_addr[i].isp_lsc_tbl_vaddr = (void*)(pa_base + ISP_LENS_MEM_OFS);
        ...
        dev->isp_tbl_addr[i].isp_gamma_tbl_vaddr = (void*)(pa_base + ISP_GAMMA_MEM_OFS);
...
static inline void vfe_set_addr(struct vfe_dev *dev,struct vfe_buffer *buffer)
{
  ...
  vfe_dbg(3,"csi_buf_addr_orginal=%lx\n", addr_org);
}
...
{
  unsigned int buffer_addr = (unsigned int)isp_gen->stat.hist_buf->stat_buf;
  isp_gen->stat.hist_buf->stat_buf = (void*) (buffer_addr);
  isp_gen->stat.ae_buf->stat_buf =  (void*) (buffer_addr + ISP_STAT_AE_MEM_OFS);  
  isp_gen->stat.awb_buf->stat_buf = (void*) (buffer_addr + ISP_STAT_AWB_MEM_SIZE);
  isp_gen->stat.af_buf->stat_buf = (void*) (buffer_addr + ISP_STAT_AF_MEM_SIZE);
  isp_gen->stat.afs_buf->stat_buf = (void*) (buffer_addr + ISP_STAT_AFS_MEM_SIZE);  
  isp_gen->stat.awb_win_buf->stat_buf = (void*) (buffer_addr + ISP_STAT_AWB_WIN_MEM_OFS);
}
...
//isp_stat_handle:
    ...
    if(dev->is_isp_used && dev->is_bayer_raw) {
      ...
      dev->isp_gen_set_pt->stat.hist_buf = &isp_stat_bq->isp_stat[stat_buf_pt->id].isp_stat_buf;
          ...
          struct v4l2_format *f)
{
  ...
  struct isp_size isp_size[ISP_MAX_CH_NUM] = {{0},{0},{0},};
  ...
  if(dev->is_isp_used && dev->is_bayer_raw) {
    switch (ctrl->id) {
      ...
      case V4L2_CID_AUTO_FOCUS_STATUS:  //Read-Only
        ...
        {
          unsigned int val=0;
          if(ret!=V4L2_AUTO_FOCUS_STATUS_BUSY)
...
static int vfe_request_pin(struct vfe_dev *dev)
{
 ...
 //pin for FPGA
  ...
  writel((volatile void*)0x33333333,(volatile void __iomem*)(0xf1c20800+0x90));
  writel((volatile void*)0x00003333,(volatile void __iomem*)(0xf1c20800+0x94));
  writel((volatile void*)((readl((volatile void __iomem*)(0xf1c20800+0x24)) & 0xff00ffff) | 0x220000),\
  																				(volatile void __iomem*)(0xf1c20800+0x24));
...
static int vfe_resource_request(struct platform_device *pdev ,struct vfe_dev *dev)
{
  ...
  void __iomem      *regs; 
	...
	char vfe_res_mipicsi[16] = "mipicsi";
	char vfe_res_mipicsi_dphy[16] = "mipicsi_dphy";
	char *vfe_res_name[] = {
		[0] = vfe_res_csi,
		[1] = vfe_res_isp,
		[2] = vfe_res_mipicsi,
		[3] = vfe_res_mipicsi_dphy,
	};
  ...
  for(i = 0, j = 0; i < pdev->num_resources; i++) {
	  ...
	  } else if (!strcmp(vfe_res_name[j], vfe_res_mipicsi)) {
	  	dev->regs.protocol_regs_res = regs_res;
      dev->regs.protocol_regs = regs;
      vfe_dbg(0,"dev->regs.protocol_regs = %p\n",dev->regs.protocol_regs);
    } else if (!strcmp(vfe_res_name[j], vfe_res_mipicsi_dphy)) {
	  	dev->regs.dphy_regs_res = regs_res;
      dev->regs.dphy_regs = regs;
      vfe_dbg(0,"dev->regs.dphy_regs = %p\n",dev->regs.dphy_regs);
    }
    j++;
  }
  
  dev->isp_load_reg_mm.size = ISP_LOAD_REG_SIZE;
  ret = os_mem_alloc(&dev->isp_load_reg_mm);
  if(!ret) {
  ...
  dev->isp_save_reg_mm.size = ISP_SAVED_REG_SIZE;
	ret = os_mem_alloc(&dev->isp_save_reg_mm);
  if(!ret) {
...
{  
  ...
  if(dev->regs.protocol_regs)
    sunxi_unmap_kernel(dev->regs.protocol_regs);
  if(dev->regs.dphy_regs)
    sunxi_unmap_kernel(dev->regs.dphy_regs); 
...
//  struct i2c_adapter *i2c_adap_act;
  ...
  if((dev->ccm_cfg[0]->is_isp_used && dev->ccm_cfg[0]->is_bayer_raw) || \
  	 (dev->ccm_cfg[1]->is_isp_used && dev->ccm_cfg[1]->is_bayer_raw))
  {
  ...
  dev_set_name(&dev->pdev->dev,"sunxi_vip.%d",dev->id);
...
static void resume_work_handle(struct work_struct *work)
{
  ...
  for (input_num=0; input_num<dev->dev_qty; input_num++) {
    ...
    ret = v4l2_subdev_call(dev->sd,core, init,0);
    if (ret!=0) {
      vfe_err("sensor full initial error at device number %d when resume from suspend!\n",input_num);
    } else {
      vfe_dbg(0,"sensor full initial at device number %d success when resume from suspend!\n",input_num);
    }
