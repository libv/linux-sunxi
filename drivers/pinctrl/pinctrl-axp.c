...
int axp_gpio_get_data(int gpio)
{
	uint8_t ret;
	...
	switch(gpio){
		case 0:axp_read(axp->dev,AXP22_GPIO01_STATE,&ret);ret &= 0x10;ret = ret>>4;break;
		case 1:axp_read(axp->dev,AXP22_GPIO01_STATE,&ret);ret &= 0x20;ret = ret>>5;break;
		...
		case 6:printk("This IO is not an input,no return value!");return -ENXIO;
		default:return -ENXIO;
	}
	return ret;
}

int axp_gpio_set_data(int gpio, int value)
{
	struct axp_dev *axp = NULL;
	...
	if(value){//high
		switch(gpio) {
			...
			case 1: 
				...
				return axp_set_bits(axp->dev,AXP22_GPIO1_CFG,0x01);
			case 2: 
				return axp_set_bits(axp->dev,AXP22_GPIO2_CFG,0x80);
			case 3: 
				axp_clr_bits(axp->dev,AXP22_GPIO3_CFG, 0x08);
				return axp_clr_bits(axp->dev,AXP22_GPIO3_CFG,0x30);
			case 4: 
				axp_clr_bits(axp->dev,AXP22_GPIO4_CFG, 0x10);
				return axp_set_bits(axp->dev,AXP22_GPIO4_STA,0x04);
			case 5:
				return axp_set_bits(axp->dev,AXP15_GPIO5_CFG,0x03);
			case 6:
				return axp_set_bits(axp->dev,AXP15_GPIO6_CFG,0x80);
			default:break;
		}
	}else{//low
		switch(gpio){
			...
			case 2: 
				return axp_clr_bits(axp->dev,AXP22_GPIO2_CFG,0x80);
			case 3: 
				axp_clr_bits(axp->dev,AXP22_GPIO3_CFG, 0x08);
			...
			case 5:
				axp_clr_bits(axp->dev,AXP15_GPIO5_CFG, 0x01);
				return axp_set_bits(axp->dev,AXP15_GPIO5_CFG,0x02);
			case 6:
				return axp_clr_bits(axp->dev,AXP15_GPIO6_CFG,0x80);
			default:break;
		}
	}
	return -ENXIO;
...
int axp_pmx_set(int gpio, int mux)
{
	struct axp_dev *axp = NULL;
	...
	if(mux == 1){//output
		...
		switch(gpio){
			...
			case 5:	printk("This IO can not config as an input!");
				return -EINVAL;
			case 6:	printk("This IO can not config as an input!");
				return -EINVAL;
			default:return -ENXIO;
		}
	}
