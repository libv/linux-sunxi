...
char	sunxi_dbg_pinname[SUNXI_MAX_NAME_LEN];
unsigned int		sunxi_dbg_function;
unsigned int		sunxi_dbg_data;
unsigned int		sunxi_dbg_level;
unsigned int		sunxi_dbg_pull;
unsigned int		sunxi_dbg_trigger;
                                                ...
                                                unsigned pin)
{
	...
	pr_warn("seach pin [%d] target bank failed\n", pin);
	return NULL;
}
			     ...
			     unsigned long *config)
{
	...
	if (IS_ERR_OR_NULL(bank)) {
		pr_warn("invalid pin number [%d] to get pinconf\n", pin);
		return -EINVAL;
	}
	...
	case SUNXI_PINCFG_TYPE_DRV:
		...
		*config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_DRV, dlevel);
	        pr_info("sunxi pconf get pin [%s] drive strength [LEVEL%d]\n", 
	...
	case SUNXI_PINCFG_TYPE_PUD:
		...
		*config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_PUD, pull);
	        pr_info("sunxi pconf get pin [%s] pull [%d]\n", 
	...
	case SUNXI_PINCFG_TYPE_DAT:
		...
		*config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_DAT, data);
	        pr_info("sunxi pconf get pin [%s] data [%d]\n", 
	...
	case SUNXI_PINCFG_TYPE_FUNC:
		...
		*config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_FUNC, func);
	        pr_info("sunxi pconf get pin [%s] func [%d]\n", 
		         pin_get_name(pctl->pctl_dev, pin), func);
		break;
	default:
		pr_warn("invalid sunxi pconf type for get\n");
		return -EINVAL;
	}
	return 0;
}
			     ...
			     unsigned long config)
{
	...
	if (IS_ERR_OR_NULL(bank)) {
		pr_warn("invalid pin number [%d] to set pinconf\n", pin);
		return -EINVAL;
	}
	switch (SUNXI_PINCFG_UNPACK_TYPE(config)) {
	case SUNXI_PINCFG_TYPE_DRV:
		...
		pr_info("sunxi pconf set pin [%s] drive strength to [LEVEL%d]\n", 
		         pin_get_name(pctl->pctl_dev, pin), dlevel);
		break;
	case SUNXI_PINCFG_TYPE_PUD:
		...
		sunxi_write_reg(val,reg);
		pr_info("sunxi pconf set pin [%s] pull to [%d]\n", 
		        pin_get_name(pctl->pctl_dev, pin), pull);
		break;
	case SUNXI_PINCFG_TYPE_DAT:
		...
		sunxi_write_reg(val,reg);
		pr_info("sunxi pconf set pin [%s] data to [%d]\n", 
		        pin_get_name(pctl->pctl_dev, pin), data);
		break;
	case SUNXI_PINCFG_TYPE_FUNC:
		...
		sunxi_write_reg(val,reg);
		pr_info("sunxi pconf set pin [%s] func to [%d]\n", 
		         pin_get_name(pctl->pctl_dev, pin), func);
		break;
	default:
		pr_warn("invalid sunxi pconf type for set\n");
		return -EINVAL;
	}
	return 0;
}
                          ...
                          u8 config,bool enable)
{
	...
	if (IS_ERR_OR_NULL(bank)) {
		pr_warn("invalid pin number [%d] to pmx set\n", pin);
		return;
	}
	...
	pr_info("sunxi pmx set pin [%s] to %d\n", 
...
static int sunxi_pinctrl_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	...
	if (IS_ERR_OR_NULL(bank)) {
		pr_warn("invalid pin number [%d] to get gpio data\n", offset);
		return -EINVAL;
	}
				   ...
				   int value)
{
	...
	if (IS_ERR_OR_NULL(bank)) {
		pr_warn("invalid pin number [%d] to set gpio data\n", offset);
		return;
	}
	reg = sunxi_data_reg(pin_bias);
	shift = sunxi_data_offset(pin_bias);
					...
					unsigned offset, unsigned value)
{
	...
	if (SUNXI_EINT_TYPE_NONE == bank->eint_type){
		pr_warn("pin bank havn't eint pin type\n");
		return -EINVAL;
	}
                                    ...
                                    char  *device_name)
{
	...
	if (!sunxi_pin_valid(pctl, cfg->gpio)) {
		pr_warn("invalid pin number under sunxi platform\n");
		return 0;
	}
	...
	if (!pin_name) {
		pr_warn("find pin [%d] name failed\n", pin_number);
		return 0;
	}
...
static int sunxi_gpio_irq_set_type(struct irq_data *irqd, unsigned int type)
{
	...
	//pr_warn("sunxi set irq %d trigger type to %d\n", irqd->irq, type);
	/* find pin name by number */
	...
	if (IS_ERR_OR_NULL(func)) {
		...
		pr_warn("the pin [%d] have no eint function\n", pin);
		return -EINVAL;
	}
	switch (type) {
	...
	sunxi_write_reg(value, bank->membase + reg_offset);
	//pr_warn("sunxi set pin [%s] to eint func\n", 
...
static int sunxi_pin_configure_show(struct seq_file *s, void *d)
{
	...
	seq_printf(s, "pin[%s] function:  %x;                 register addr: 0x%8x\n"
		, sunxi_dbg_pinname,function,(unsigned int)(bank->membase + sunxi_mux_reg(pin_bias)));
	/*get pin data*/
	value = readl(bank->membase + sunxi_data_reg(pin_bias));
	data = (value >> sunxi_data_offset(pin_bias)) & DATA_PINS_MASK;
	seq_printf(s, "pin[%s] data:  %x(default value : 0);  register addr: 0x%8x\n"
		, sunxi_dbg_pinname,data,(unsigned int)(bank->membase + sunxi_data_reg(pin_bias)));
	/*get pin dlevel*/
	value = readl(bank->membase + sunxi_dlevel_reg(pin_bias));
	dlevel = (value >> sunxi_dlevel_offset(pin_bias)) & DLEVEL_PINS_MASK;
	seq_printf(s, "pin[%s] dleve: %x(default value : 1);  register addr: 0x%8x\n"
		, sunxi_dbg_pinname,dlevel,(unsigned int)(bank->membase + sunxi_dlevel_reg(pin_bias)));
	/*get pin pull*/
	value = readl(bank->membase + sunxi_pull_reg(pin_bias));
	pull = (value >> sunxi_pull_offset(pin_bias)) & PULL_PINS_MASK;
	seq_printf(s, "pin[%s] pull:  %x(default value : 0);  register addr: 0x%8x\n"
		, sunxi_dbg_pinname,pull,(unsigned int)(bank->membase + sunxi_pull_reg(pin_bias)));
	...
	if ( 6 == function){
		...
		seq_printf(s, "\npin[%s] trigger:  %x;                  register addr: 0x%8x\n"
			, sunxi_dbg_pinname,trigger,(unsigned int)(bank->membase + reg_offset));
		...
		seq_printf(s, "pin[%s] mask :     %x(0:disable 1:enable);        register addr: 0x%8x\n"
			, sunxi_dbg_pinname,mask,(unsigned int)(bank->membase + reg_offset));
		...
		seq_printf(s, "pin[%s] pending :  %x(0:No Pending 1:Pending);    register addr: 0x%8x\n"
			, sunxi_dbg_pinname,pending,(unsigned int)(bank->membase + reg_offset));
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (0 <= function && 7>= function)
		...
		pr_warn("Input Parameters function error!\n");
		return -EINVAL;
	}
	if (0 <= data && 1>= data)
		sunxi_dbg_data = data;
	else{
		pr_warn("Input Parameters data error!\n");
		return -EINVAL;
	}
	if (0 <= pull && 3>= pull)
		sunxi_dbg_pull = pull;
	else{
		pr_warn("Input Parameters pull error!\n");
		return -EINVAL;
	}
	if (0 <= dlevel && 3>= dlevel)
		sunxi_dbg_level = dlevel;
	else{
		pr_warn("Input Parameters dlevel error!\n");
		return -EINVAL;
	}
	if (0 <= trigger && 7>= trigger)
		sunxi_dbg_trigger = trigger;
	else{
		pr_warn("Input Parameters trigger error!\n");
		return -EINVAL;
	}
	pctldev = get_pinctrl_dev_from_devname(dev_name);
	if (!pctldev) {
	...
	pr_info("pin[%s] set function:     %x;     register addr: 0x%8x\n"
		, sunxi_dbg_pinname,function,(unsigned int)(bank->membase + sunxi_mux_reg(pin_bias)));
	/* set data value*/
	value = readl(bank->membase + sunxi_data_reg(pin_bias));
	mask = DATA_PINS_MASK << sunxi_data_offset(pin_bias);
	...
	pr_info("pin[%s] set data:         %x;     register addr: 0x%8x\n"
		, sunxi_dbg_pinname,data,(unsigned int)(bank->membase + sunxi_data_reg(pin_bias)));
	/* set dlevel value*/
	value = readl(bank->membase + sunxi_dlevel_reg(pin_bias));
	mask = DLEVEL_PINS_MASK << sunxi_dlevel_offset(pin_bias);
	...
	pr_info("pin[%s] set dleve:        %x;     register addr: 0x%8x\n"
		, sunxi_dbg_pinname,dlevel,(unsigned int)(bank->membase + sunxi_dlevel_reg(pin_bias)));
	/* set pull value*/
	value = readl(bank->membase + sunxi_pull_reg(pin_bias));
	mask = PULL_PINS_MASK << sunxi_pull_offset(pin_bias);
	...
	pr_info("pin[%s] set pull:         %x;     register addr: 0x%8x\n"
		, sunxi_dbg_pinname,pull,(unsigned int)(bank->membase + sunxi_pull_reg(pin_bias)));
	if (6 == function ){
		reg_offset = sunxi_eint_cfg_reg(pin_bias);
		shift = sunxi_eint_cfg_offset(pin_bias);
		...
		value |= trigger << shift;
		sunxi_write_reg(value, bank->membase + reg_offset);
		pr_info("pin[%s] set trigger :     %x;     register addr: 0x%8x\n"
			, sunxi_dbg_pinname,trigger,(unsigned int)(bank->membase + reg_offset));
		return count;
	}
	return count;
...
static int __init sunxi_pinctrl_init(void)
{
	...
	if (IS_ERR_VALUE(ret)) {
		pr_warn("register sunxi platform device failed\n");
		return -EINVAL;
	}
