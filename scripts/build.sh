...
build_mdfs()
{
	echo "build mdfs"
	make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} KDIR=${LICHEE_KDIR} \
		-C ${LICHEE_KDIR}/arch/arm/mach-$platform_name/dram-freq/mdfs all
}
...
build_nand_lib()
{
	echo "build nand library ${NAND_ROOT}/${chip_name}/lib"
	if [ -d ${NAND_ROOT}/${chip_name}/lib ]; then
		echo "build nand library now"
	make -C modules/nand/${chip_name}/lib clean	2>/dev/null
	make -C modules/nand/${chip_name}/lib lib install
	else
...
build_kernel()
{
	cd ${LICHEE_KDIR}
	if [ ! -e .config ]; then
		echo -e "\n\t\tUsing default config(${chip_name}smp_${platform}_defconfig)... ...!\n"
		cp arch/arm/configs/${chip_name}smp_${platform}_defconfig .config
	fi

	# remove all *.ko files before compiling
	echo "Clear all *.ko files before compiling ..."
	find . -name *.ko -delete
	...
	#build_standby
	#build_mdfs
	make ARCH=arm CROSS_COMPILE=${CROSS_COMPILE} -j24 uImage modules
...
#	cp drivers/net/wireless/bcm4330/firmware/nvram.txt ${LICHEE_MOD_DIR}/bcm4330_nvram.txt
	...
	echo "$chip_name compile successful"
}
...
#
	...
	make -C modules/nand LICHEE_MOD_DIR=${LICHEE_MOD_DIR} LICHEE_KDIR=${LICHEE_KDIR} \
		CONFIG_CHIP_ID=${CONFIG_CHIP_ID} install
	if [ x$platform = xdragonboard ];then
		copy_nand_mod
	fi
...
regen_rootfs_cpio()
{
	...
	if [ -x "./scripts/build_rootfs.sh" ]; then
		...
		cp -a ${LICHEE_MOD_DIR} $LICHEE_KDIR/skel/lib/modules
        fi
	if [ $? -ne 0 ]; then
		echo "copy module error: $?"
		exit 1
	fi
	./scripts/build_rootfs.sh c ./output/rootfs.cpio.gz >/dev/null
}
...
build_ramfs()
{
	...
	# regenerate bImage with new rootfs.cpio.gz
#	build_kernel
...
kernel)
	...
	;;
modules)
	if [ ! "x${chip_name}" = "xsun9iw1p1" ];then
		build_modules
	fi
	;;
clean)
