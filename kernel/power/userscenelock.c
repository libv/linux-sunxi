...
struct rb_root user_scene_locks;
	...
	struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (!check_scene_locked(SCENE_USB_STANDBY)) {
		s += scnprintf(s, end - s, "%s ", "usb_standby");
	}
	if (!check_scene_locked(SCENE_TALKING_STANDBY)) {
		s += scnprintf(s, end - s, "%s ", "talking_standby");
	}
	if (!check_scene_locked(SCENE_MP3_STANDBY)) {
		s += scnprintf(s, end - s, "%s ", "mp3_standby");
	}
	s += scnprintf(s, end - s, "\n");
